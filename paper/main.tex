...
\begin{abstract}
Knowledge distillation is the standard route from large encrypted-traffic classifiers to the small models that can run at the network edge, and the literature evaluates the result almost exclusively by its accuracy. We ask what else a distilled student inherits from its teacher: its detection of unknown traffic, its calibration, and its reliance on shortcut features, and whether the inheritance lasts. On CESNET-TLS-Year22, a year of real TLS traffic, we distil a 101k-parameter student from two teachers of equal accuracy that differ in ensembling and capacity---a five-member ensemble and a single wider model---so that a student's per-flow behaviour can be attributed to the specific teacher that produced its soft targets rather than to soft labels in general. The design was pre-registered and publicly registered before any test result was computed; ten hypotheses were tested on 18 test windows spanning 35 weeks after training, with a day-by-service cluster bootstrap and Holm correction. Two are supported. A student's per-flow unknown-scores shift measurably toward its own teacher, but only at a conventional distillation temperature---an arm added before the design was frozen, though after validation data had been seen; the accuracy-optimal temperature transmits nothing teacher-specific. A teacher that relies on a shortcut passes its over-confidence to a student that never sees the feature. Under the pre-registered logit-based scores nothing else transfers: against the same student trained directly and temperature-scaled, distillation improves no unknown-detection outcome, and against label smoothing it trades detection for calibration. An exploratory analysis added at review shows that this is in part the scoring rule's doing---with a feature-space detector the teacher is $0.073$ AUROC better than the direct student, where the energy score sees $0.001$, and the conventional-temperature student inherits most of that advantage---but also that plain label smoothing recovers more of it than distillation does. Two hypotheses are reversed. The teacher--student gap in unknown detection does not widen under either score; under the energy score it reverses, so that after about three months the 101k student scores as the better detector of the two, while the co-primary MSP score shows no such crossing. Shortcut reliance is set by model size, not by distillation: both small students lean on a planted feature about twice as hard as the teacher regardless of how they were trained. Distillation transfers the teacher's habits; what looks like an inherited ability is available without a teacher.
\end{abstract}
...
\begin{table*}[t]
\centering
\caption{The ten pre-registered hypotheses. Estimates are pooled over 18 test windows; intervals are 95\% percentile intervals from 1,000 day$\times$service cluster bootstrap resamples. All tests are one-sided, so H4's rows report the one-sided lower confidence bound from paired $t$-tests over six ($\rho$, seed) pairs, and the two-sided percentile intervals of the other rows may be read the same way. $p$-values are Holm-corrected over these ten hypotheses and no others; with 1,000 resamples the smallest attainable one-sided $p$ is $1/1001 = 0.000999$, so 0.009 is the smallest Holm-corrected value a family of ten can produce, and rows shown as 1.0 are bounded rather than precisely measured. The $T=4$ arm that H1 tests (\kd{kdA4}, \kd{kdB4}) was added to the design before the freeze but after the validation weeks had shown that distillation at the accuracy-optimal temperature transfers nothing teacher-specific; the $T=1$ arm is reported beside it in Fig.~\ref{fig:specificity}. Both sensitivity analyses reach the same decision on every row.}
...
\section{Additional Analyses (Exploratory, Added in Revision)}
\label{sec:exploratory}

The analyses in this section were added after the confirmatory results were known, at peer review. None of them is pre-registered, none shares a multiple-comparison family with the ten hypotheses of Table~\ref{tab:hypotheses}, and none alters a number reported above: the frozen analysis code was re-run in the revision environment and reproduces the confirmatory tables exactly. Intervals come from the same day$\times$service cluster bootstrap, resampling training seeds in the same way, so that they can be read against the confirmatory intervals.

\input{exploratory_tables}

\subsection{A Dedicated Open-Set Detector}
\label{sec:featurescores}

The confirmatory tests score unknown traffic from the logits alone. Under those scores Teacher A and the directly trained student are separated by 0.001 energy AUROC, so H2 and H5 compare students in a regime where the teacher has no detection advantage to pass on. To find out whether that is a property of distillation or of the scoring rule, we scored every model already trained with two detectors built on its penultimate features---a Mahalanobis distance to the nearest class mean under a tied, shrunk covariance, and the distance to the $10$th nearest training flow. No model was retrained, and both scores are computed from checkpoints and evaluation windows that already existed. Table~\ref{tab:featurescores} reports them beside the pre-registered scores.

The answer is that it was the scoring rule. Averaged over the 18 test windows, Teacher A detects unknown traffic $0.073$ AUROC better than the directly trained student under the Mahalanobis score and $0.034$ better under the feature $k$-NN score, against $0.001$ under the energy score. A teacher advantage of the size the reviewer's objection presumes does exist; it is simply invisible to a logit-based score.

What the student does with that advantage divides sharply by temperature. The conventional-temperature student \kd{kdA4} recovers almost all of it ($+0.076$ Mahalanobis AUROC over \kd{direct}, against the $+0.073$ available), while the accuracy-optimal \kd{kdA} recovers none ($-0.003$). This is the same split the teacher-swap test finds (H1), now visible in an outcome an operator would care about rather than in a rank correlation, and it strengthens rather than weakens the paper's reading of H2: the tuned arm that H2 tests transfers nothing because its targets are nearly one-hot, not because there was nothing to transfer.

Two observations keep this from being an argument for distillation. First, label smoothing---which involves no teacher at all---transfers more ($+0.088$) than distillation does, so the pre-registered conclusion that a cheaper teacher-free control matches or beats distillation survives in the regime where a real advantage exists. Second, the temperature-scaled student \kd{directTS} is identical to \kd{direct} under these scores by construction, since temperature scaling rescales logits and leaves the penultimate features untouched; the feature-space comparison therefore has one fewer control than the logit-space one.

A caution on reading Table~\ref{tab:featurescores} across rows: the teachers' features are 600- and 1{,}200-dimensional and the students' are 128-dimensional, and a Mahalanobis distance is not scale-free across dimensionality. Teacher-versus-student rows should be read as indicative. The student-versus-student rows, which carry the argument above, compare models of identical architecture and are not affected.

Finally, the ensemble-distribution-distillation student, the worst detector in the grid under the energy score ($-0.092$ against \kd{kdA}), is among the best under both feature-space scores ($+0.077$ and $+0.043$ against \kd{kdA}). That is consistent with the mechanism proposed in Section~\ref{sec:discussion}: the proxy-Dirichlet objective caps the total concentration, which distorts exactly the logit scale that the energy score reads, while leaving the representation intact. H5's negative result should be read as specific to logit-based scores.

\subsection{Separating Teacher Identity from Ensembling, Width and Family}
\label{sec:swaps}
\draftnote{\kd{kdM0}/\kd{kdM1} single-member swaps, the transformer Teacher C, and the \kd{hardA} label-copying anchor.}

\subsection{How Large Is a Teacher Preference by Chance?}
\draftnote{Spread of an undistilled student's correlation with two equally good same-family teachers.}

\subsection{Does the Ensemble Age Faster than Its Members?}
\draftnote{Per-member versus ensemble detection slopes, both scores.}

\subsection{Dependence Between Start Dates}
\draftnote{Per-start H3 slopes and the calendar-overlap table.}

\subsection{Student Capacity}
\draftnote{Widths 16 and 96 beside the registered 48.}
