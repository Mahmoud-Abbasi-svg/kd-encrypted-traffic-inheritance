...
Under MSP it is a different matter. The ensemble ages at $-0.00306$ per week against its members' $-0.00282$, an excess of $0.00023$, and the MSP gap narrows at only $0.00030$ per week. Aggregation could therefore account for most of the MSP narrowing, and we cannot rule it out with these data. The honest statement is that the mechanism is settled for one co-primary score and open for the other, which is the same asymmetry the drift result itself shows.
...
Distillation does not close the gap at any width. Table~\ref{tab:logitscores} measures every row against the registered width-48 \kd{direct} student, so the equal-width comparison is a difference of two of its rows: under the energy score \kd{kdA4} is worse than the directly trained student of its own size by $0.030$ at width 16, $0.026$ at width 48 and $0.017$ at width 96. The penalty is close to flat across the range rather than growing as the student shrinks. The shortcut experiment's conclusion is stated for a single student size in Section~\ref{sec:discussion}; this sweep shows that the relevant property, a small student detecting unknowns at least as well as distillation makes it, holds across a sixfold range of student width.
