...
The feature-space scores of analysis 1 were extended to `kdF`, `hardA`, `kdM0` and `kdM1` on 23 September, with no model retrained, and the exploratory analysis was re-run over the enlarged set (`results/exploratory/REVISION_FULL2/`). The statistics that both passes share are unchanged: all six CSV files common to `REVISION_FULL` and `REVISION_FULL2` — the swap shifts, both drift tables, the per-start H3 slopes, the calendar overlap and the teacher-preference null — are byte-identical, and the 68 detection-advantage rows the two have in common agree exactly.

Two corrections to the *exploratory* code followed, neither touching the frozen analysis and neither changing an estimate.

- **Unit counts were over-reported in `detection_advantage.csv`.** Every row recorded the length of the unit list, 18, although the pooled estimate is a `nanmean` over units and the conditions that ran on one start date contribute only the nine windows that start date tests. The estimates were therefore correct and the counts beside them were not, for the four width-sweep rows, `kdC` and `kdF`. `cluster_bootstrap_many` now counts the units each statistic actually used and reports that. Re-running the point estimates with the corrected code reproduces all 78 estimates of `REVISION_FULL2` to the last bit (`results/exploratory/UNITCOUNT/`), so the fix is one of reporting only.
- **`kdF` against `kdM0` is now bootstrapped as a paired difference.** Both arms distil from ensemble member 0 and differ only in whether they match its representation or its logits, but `kdF` pools over nine windows and `kdM0` over eighteen, so their separate intervals do not answer whether feature distillation transfers more or less than logit distillation. The paired statistic is evaluated only on the nine windows both arms share, following the existing `ls`-against-`kdA4` row. The re-run is `results/exploratory/REVISION_FULL3/`, and it is the run the paper reports.

**25 Sep 2026 — a storage-precision defect found in the deposited score files.** Two audit rounds over the manuscript found that `scripts/06_track_a.py` casts the maximum softmax probability to `float16` before storing it (line 216). `float16` cannot represent a value between 0.9995 and 1, so every flow a model scores above that collapses onto exactly 1.0: 48% of Teacher B's flows, 40% of `kdA4`'s, 20% of `direct`'s and none of `ls`'s, which label smoothing prevents from saturating. The resulting ties deflate AUROC in proportion to a model's confidence.

The consequences, established by recomputing from the full-precision `metrics.csv` written at evaluation time:

- **No confirmatory outcome changes.** H2[MSP]'s components move from $+0.00621$ to $+0.00711$ and from $-0.00225$ to $-0.00186$; H5[MSP]'s from $-0.00757$ to $-0.00847$. All keep their sign and their decision. The hypotheses compare students of similar confidence, so the bias largely cancels.
- **The energy score is unaffected.** Its values do not pile up against a representable ceiling; per-window AUROC differs by less than 0.0001, and the feature-space scores were deliberately stored as `float32` (`kdtraffic/openset.py`).
- **The exploratory MSP column is biased**, because it compares teachers against students, whose saturation fractions differ by more than a factor of two. Two figures in Section V-A are corrected to their full-precision values ($-0.020$ and $+0.012$, from $-0.027$ and $+0.020$), the MSP rows of the ageing table are refitted at full precision, and the remaining MSP intervals are labelled indicative.

The defect is in storage, not in the analysis: the evaluation that produced `metrics.csv` used full precision throughout. The score files in the Zenodo deposit carry the quantisation, which the paper now states as a limitation. `06_track_a.py` is not corrected retrospectively, because doing so would require re-running inference for every model and window and would change stored artefacts that the confirmatory analysis reads.
